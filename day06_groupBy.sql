...
select urun_adi, max(urun_miktari) as max_urun_miktari,min(urun_miktari) as min_urun_miktari from manav
where urun_Adi is not null
group by urun_adi
...
select cinsiyet, round(avg(maas),02)
from personel
group by cinsiyet;
...
select ulke, cinsiyet, avg(yas), count(*) from personel
where maas>30000
group by ulke,cinsiyet;
